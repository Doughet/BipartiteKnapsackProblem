...
public class TruckOptimisationSolver {
    private double P;
    //Total Palettes
    private double N;
    //Poids par camion
    ...
    private List<TruckOptimisationSolver.SolutionEntry> solutionEntries;
    ...
    public TruckOptimisationSolver(double p, double n, double p_PER_C, double n_PER_C,
                                   int c2,
                                   double PSalumis, double NSalumis,
    ...
    private void initSalumis(){
        for (int i = 0; i < valuesArray.length; i++) {
            if(valuesArray[i][3] == 2){
                NSalumis += valuesArray[i][0];
                PSalumis += valuesArray[i][0] * valuesArray[i][1];

                //ADD TO LIST
                String code = infoArray[i][0];
                codesSelected.put(code, valuesArray[i][0]);
            }
        }
    }
    ...
    public List<TruckOptimisationSolver.SolutionEntry> getSolution(){
        if(isFeasible){
    ...
    private void processSolutionEntries(){
        for (int i = 0; i < infoArray.length; i++) {
            if(codesSelected.containsKey(infoArray[i][0])){
                solutionEntries.add(new TruckOptimisationSolver.SolutionEntry(infoArray[i][0], infoArray[i][1], codesSelected.get(infoArray[i][0]), valuesArray[i]));
            }
        }
    }
