...
public class TruckOptimizationWriter {

    private final List<TruckOptimizationSolver.SolutionEntry> solutionEntries;
    ...
    private void WriteAdvancedTrucks(Workbook workbook) {
        ...
        int rowNum = 1;
        for (TruckOptimizationSolver.SolutionEntry rowData : solutionEntries) {
    ...
    private void WriteRemainingTrucks(Workbook workbook) {
        ...
        for (int i = 0; i < infoArray.length; i++) {
            int finalI = i;
            Optional<TruckOptimizationSolver.SolutionEntry> entry = solutionEntries.stream().filter(x -> x.code() == infoArray[finalI][0]).findFirst();
            ...
            if(!entry.isEmpty() && entry.get().amount() == valuesArray[i][0]) {
                ...
                entry = Optional.of(new TruckOptimizationSolver.SolutionEntry("", "", 0, null));
            }
