...
public class TruckAnticipationWriter {

    private final List<TruckAnticipationSolver.SolutionEntry> solutionEntries;
    ...
    public TruckAnticipationWriter(List<TruckAnticipationSolver.SolutionEntry> solutionEntries, String[][] infoArray, double[][] valuesArray, double N, double P, int C1, int C2){
    ...
    private void WriteAdvancedTrucks(Workbook workbook) {
        ...
        int rowNum = 1;
        for (TruckAnticipationSolver.SolutionEntry rowData : solutionEntries) {
    ...
    private void WriteRemainingTrucks(Workbook workbook) {
        ...
        for (int i = 0; i < infoArray.length; i++) {
            int finalI = i;
            Optional<TruckAnticipationSolver.SolutionEntry> entry = solutionEntries.stream().filter(x -> x.code() == infoArray[finalI][0]).findFirst();
            ...
            if(!entry.isEmpty() && entry.get().amount() == valuesArray[i][0]) {
                ...
                entry = Optional.of(new TruckAnticipationSolver.SolutionEntry("", "", 0, null));
            }
