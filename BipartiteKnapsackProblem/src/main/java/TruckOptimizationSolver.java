...
public class TruckOptimizationSolver {
    private double P;
    //Total Palettes
    private double N;
    //Poids par camion
    ...
    private List<TruckOptimizationSolver.SolutionEntry> solutionEntries;
    ...
    public TruckOptimizationSolver(double p, double n, double p_PER_C, double n_PER_C,
                                   int c2,
    ...
    private void processSolutionEntries(){
        for (int i = 0; i < infoArray.length; i++) {
            if(codesSelected.containsKey(infoArray[i][0])){
                solutionEntries.add(new TruckOptimizationSolver.SolutionEntry(infoArray[i][0], infoArray[i][1], codesSelected.get(infoArray[i][0]), valuesArray[i]));
            }
        }
    }
